...
PCs: `c-pc-quasi` (Quasimodo d'Clopin, Pedro Silva) · `c-pc-lucan` (Lucan "Anzol" Bruma, Renata) · `c-pc-isco` (Ruín "Isco" Bruma, Pedro Ropio) · `c-pc-drow` (4.ª jogadora, entra na sessão 4 como prisioneira da patrulha imperial; ficha e nome por chegar) · pistas `a-pc-quasi` `a-pc-lucan` `a-pc-isco` · ambições `amb-quasi-*` `amb-lucan-*` `amb-isco-*`
...
- **Sessão 4 (preparação), a patrulha do Forte Ventura** (`data/campaign/*/patrulha.json`): `c-vasco-corvelo` `c-gilberto-mendanha` `c-nalia-vasques` `c-elmiro-trigueiro` `c-rosinda-sousela` `c-damiao-prestes` `c-bento-ruivo` (o morto da jangada em B2) · beats `b-0-pressagios-do-forte` (o relógio da Bolbara à vista: aves no dia 2, tempestade parada no dia 3, fumo no dia 4) e `b-0-patrulha-imperio` (o encontro garantido do d20 = 18 no fim da sessão 3) · **`c-pc-drow`** é um *placeholder* da personagem da Camila, a 4.ª jogadora (renomear para `c-pc-<nome>` quando a ficha existir, e acrescentar pista `a-pc-<nome>` e ambições). Preparação revista com o cânone da sessão 3: `docs/sessoes/sessao-04-preparacao.md`; propostas discutidas e ainda por aplicar em `docs/sessoes/sessao-04-propostas-por-aplicar.md`; handouts: ordem de marcha e folha do acólito.
