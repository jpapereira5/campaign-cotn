...
- **Sessão 4 (preparação), a patrulha do Forte Ventura** (`data/campaign/*/patrulha.json`): `c-vasco-corvelo` `c-gilberto-mendanha` `c-nalia-vasques` `c-elmiro-trigueiro` `c-rosinda-sousela` `c-damiao-prestes` `c-bento-ruivo` (o morto da jangada em B2) · beat `b-0-patrulha-imperio` (o encontro garantido do d20 = 18 no fim da sessão 3) · **`c-pc-drow`** é um *placeholder* da personagem da Camila, a 4.ª jogadora (renomear para `c-pc-<nome>` quando a ficha existir, e acrescentar pista `a-pc-<nome>` e ambições). Preparação revista com o cânone da sessão 3: `docs/sessoes/sessao-04-preparacao.md`; handouts: ordem de marcha e folha do acólito.
